				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					if(getDolGlobalInt('INVOICE_INCLUDE_DETAILS_OF_LOTS_SERIALS')):
						...
						if(isset($object->linkedObjects['shipping']) && !empty($object->linkedObjects['shipping'])):
							...
							foreach($object->linkedObjects['shipping'] as $ship_id => $ship_infos):

								if(!empty($ship_infos->lines)):
									foreach($ship_infos->lines as $shipline_key => $shipline):
										if($object->lines[$i]->fk_product != $shipline->fk_product): continue; endif;
										if(!empty($shipline->detail_batch)):
											$batch_content = '';
											foreach ($shipline->detail_batch as $linebatch_key => $linebatch):
												$batch_content .= $langs->trans('Batch').': '.$linebatch->batch;
												$batch_content .= ' - '.$langs->trans('Qty').': '.$linebatch->qty;
												$batch_content .= '<br>';
											endforeach;
											$pdf->writeHTMLCell($this->posxpicture - $curX - $progress_width, 3, $curX, $posYAfterDescription, $outputlangs->convToOutputCharset($batch_content), 0, 1, false, true, 'J', true);
										endif;
									endforeach;
								endif;
							endforeach;
